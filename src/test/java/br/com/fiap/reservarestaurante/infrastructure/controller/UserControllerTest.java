...
class UserControllerTest {
    ...
    void shouldFindUser() throws Exception {
        ...
        var user = new UserDTO(id, "a", "va@fiap.com.br");
        var objMapper = new ObjectMapper();
        var response = objMapper.writeValueAsString(user);
        when(findUserUseCase.execute("va@fiap.com.br")).thenReturn(user);
        this.mockMvc.perform(get("/users/va@fiap.com.br")).andDo(print()).andExpect(status().isOk())
    ...
    void shouldCreateUser() throws Exception {
        var req = new CreateUserDTO("abc", "bcd", 34, "va@fiap.com.br");
        var user = new User("abc", "bcd", 34, "va@fiap.com.br");
        var objMapper = new ObjectMapper();
